...
* That extra $a(1-a)$ term causes the **Vanishing Gradient Problem**—if the neuron is "wrong" with high confidence (e.g., $a \approx 1, y=0$), the gradient becomes zero, and the network refuses to learn. The Log Loss formulation prevents this by ensuring the gradient is just $a-y$.
