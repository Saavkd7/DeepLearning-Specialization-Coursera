...
If you feed a matrix of shape $(m, n_x)$ into this architecture, the matrix multiplication $W X$ will fail due to dimension mismatch (Inner dimensions must match: $(n^{[1]}, n^{[0]}) \times (n^{[0]}, m)$). Always ensure your examples are stacked as **columns**.
