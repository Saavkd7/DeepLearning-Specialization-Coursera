...
## 1. Executive Summary
To train a neural network using **Backpropagation**, we must calculate the **gradient** (slope) of the loss function with respect to the weights. Since weights affect the output through the activation function $g(z)$, we need to compute the derivative $g'(z)$ (or $\frac{d}{dz}g(z)$). This slope tells the optimizer: *"If I nudge the input $z$ slightly, how much will the activation $a$ change?"*.
...
* **Derivative Formula:**
  $$g'(z) = g(z)(1 - g(z)) = a(1 - a)$$
* **Mechanics:**
    * If $z = 10$ (large), $a \approx 1$. Derivative $\approx 1(1-1) = 0$. (Slope is flat).
    * If $z = 0$, $a = 0.5$. Derivative $= 0.5(1-0.5) = 0.25$ (Max slope).
* **Efficiency:** We can calculate the slope directly from the activation $a$ without recomputing exponentials.
...
* **Derivative Formula:**
  $$g'(z) = 1 - (g(z))^2 = 1 - a^2$$
* **Mechanics:**
    * If $z = 10$, $a \approx 1$. Derivative $\approx 1 - 1^2 = 0$.
    * If $z = 0$, $a = 0$. Derivative $= 1 - 0 = 1$ (Steeper than Sigmoid).
* **Efficiency:** Like Sigmoid, the slope is computed purely from the output $a$.
...
* **Derivative Formula:**
  ...
  \end{cases}
  $$
* **Mechanics:** Constant gradient of 1 when active. This prevents the vanishing gradient problem.
* **Edge Case:** At exactly $z=0$, the derivative is mathematically undefined. In code, we arbitrarily set it to 1 (or 0); it doesn't matter practically.
...
* **Derivative Formula:**
  ...
  \end{cases}
  $$
* **Mechanics:** Allows a tiny "leak" of gradient even for negative values, preventing "dead neurons".
