...
## 1. Executive Summary
To train a neural network using **Backpropagation**, we must calculate the **gradient** (slope) of the loss function with respect to the weights. Since weights affect the output through the activation function $g(z)$, we need to compute the derivative $g'(z)$ (or $\frac{d}{dz}g(z)$).
